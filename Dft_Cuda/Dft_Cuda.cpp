...
int main(void)
{
	...
	std::cout << "device maxThreadsDim: " << props.maxThreadsDim[0] << "|" << props.maxThreadsDim[1] << "|" << props.maxThreadsDim[2] << std::endl;
		...
		*mv = v;
		...
		for (int u = 0; u < w; u++) {
			...
			*mu = u;
			...
			// launch cuda kernel
			host_sumImageLine(dev_image, w, dev_w, dev_h, dev_u, dev_v, dev_sumLines);
